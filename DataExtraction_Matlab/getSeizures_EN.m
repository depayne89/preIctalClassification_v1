...
%% start grabbing data
N = length(SzInd);
preIctal = zeros(Tbefore,N);
IctalDropouts = zeros(Tbefore,N);
IctalCirc = zeros(1,N);
...
for n = 1:N
    ...
    t0 = SzTimes(SzInd(n))/1e6 - Tbefore - Toffset;
...
%% Displaying example traces
    downed = downsample(Data, 400/fr);
    
    view_len = size(downed,1)
    t = 1:view_len; %Close enough to seconds
